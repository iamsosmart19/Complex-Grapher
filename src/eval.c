...
/* 	cplx ret = 0; */
/* 	/1* for(int i = 0; i < tknCnt; i++) { *1/ */
/* 	/1* 	if(cimag(arr[i]) == DBL_MAX) { *1/ */
/* 	/1* 		printf("FUNC: %s\n", lookuptable[(int)creal(arr[i])]); *1/ */
/* 	/1* 	} *1/ */
/* 	/1* 	else if(cimag(arr[i]) == -DBL_MAX) { *1/ */
/* 	/1* 		char table[6] = {'+', '-', '*', '/', '^', '_'}; *1/ */
/* 	/1* 		printf("OP: %c\n", table[(int)creal(arr[i])]); *1/ */
/* 	/1* 	} *1/ */
/* 	/1* 	else { *1/ */
